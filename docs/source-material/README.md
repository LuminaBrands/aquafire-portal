...
| `note-installer-field-tips.txt` | Installer field practice on framing, sealing and downdraft prevention (Lumina Brands, Aug 2026, from senior installer Brad) — cutout clearance of 3/8" **in total** (nominal + 3/8", split across the two outside edges), the tape fallback when the enclosure isn't drywalled (all open edges underneath plus the front and back side vents — **not** for media-topped installs), sealing a Direct Plumb Kit install, open-plenum HVAC, keeping the top clear. Published as the "Installer Field Notes" section of `enclosure-guide.html`. **That width was adopted as the portal's own figure**, so `app.js` / `builder.js` deliberately report 1/8" wider than the + 1/4" in the 2026 spec guides below — see the DECISION block in the note, and the RE-RESOLVED entry recording that the 3/8" is the total across the opening, not per side. |
...
| Aug 2026 | **"DP kit" = Direct Plumb Kit**, not "downdraft prevention kit" — the original write-up of the installer conversation carried the wrong expansion, and an earlier revision of the note and the portal repeated it. The sealing advice belongs to Aquafire Original installs with a Direct Plumb Kit. Also recorded in the same pass: the tape seals **all open edges underneath plus the front and back side vents**, and the method is **not for media-topped installs** (the top vents become the intake). | `note-installer-field-tips.txt` |
| Aug 2026 | *(Superseded by the row below.)* Installer cutout clearance clarified per Lumina Brands: the **⅜″ is per outside edge (per side)**, so a cutout is **nominal + ¾″** overall, not + ⅜″. An earlier revision of the note and of the portal read it as ⅜″ overall; `MODELS` in `app.js` / `builder.js` and all customer-facing copy were corrected. The 2026 spec guides still publish + ¼″ overall — that divergence is intentional, not an error. | `note-installer-field-tips.txt` |
| Aug 2026 | Installer cutout clearance **re-corrected per Lumina Brands (Stefan): the ⅜″ is the TOTAL across the opening** — which always includes the two (all) outside edges — so a cutout is **nominal + ⅜″**, not + ¾″. The per-side reading in the row above was a miscommunication; the original ⅜″-overall reading was right. `MODELS` in `app.js` / `builder.js` and all customer-facing copy were corrected back. The 2026 spec guides still publish + ¼″ overall — that divergence remains intentional. | `note-installer-field-tips.txt` |
