...
| `note-installer-field-tips.txt` | Installer field practice on framing, sealing and downdraft prevention (Lumina Brands, Aug 2026, from senior installer Brad) — cutout clearance of 3/8" **per side** (nominal + 3/4" overall), the tape fallback when the enclosure isn't drywalled (all open edges underneath plus the front and back side vents — **not** for media-topped installs), sealing a Direct Plumb Kit install, open-plenum HVAC, keeping the top clear. Published as the "Installer Field Notes" section of `enclosure-guide.html`. **That width was adopted as the portal's own figure**, so `app.js` / `builder.js` deliberately report 1/2" wider than the + 1/4" in the 2026 spec guides below — see the DECISION block in the note, and the RESOLVED entry recording that the 3/8" is per outside edge, not overall. |
