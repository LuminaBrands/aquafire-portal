...
| `admin.html` | **Internal** admin homepage — big-card launcher for the three tools below (no gate of its own: it holds no data, every destination gates itself; noindexed like the rest) |
...
| `admin-theme.css` | **Admin light theme** — rebinds the admin pages' 2025-palette tokens for `data-theme="light"` and patches hub.css's nav literals (incl. the white-logo invert filter). Loaded by all four admin pages after hub.css; dark stays the default in each page's own `:root`. The toggle + `aquafire-theme` persistence live in `admin-nav.js`, and the storage key is shared with the customer portal so the choice follows the team across surfaces |
...
| `admin-nav.js` | **Shared admin header** for the three internal pages — injects a `.site-nav`-styled bar (deliberately the pre-redesign hub.css look, so admin reads as a different surface than the customer portal) with Home (`admin.html`) / Chat Admin / Dealer Portal / Help Center links, a per-page frontend action (View Help Center / View Dealer Map new-tab links; Test Chat on chat-insights pops the real widget, which that page mounts in test trim — telemetry/Slack/Mailchimp/nudge all off via `AQUAFIRE_ASSISTANT_CONFIG`, AI endpoint live) + an auth-aware Sign in/Sign out button riding the page's own Firebase session, and the light/dark toggle (persisted to the portal's shared `aquafire-theme` key; light bindings in `admin-theme.css`). One `<script defer>` tag per page; the pages' own gates stay in charge of sign-in. Uses its own `.nav-inner` width rules because every admin page redefines `.container` locally |
...
| `robots.txt` | Keeps `admin.html`, `chat-insights.html`, `dealer-admin.html`, `help-admin.html`, the parked `builder.html`, and `/api/` out of search indexes |
