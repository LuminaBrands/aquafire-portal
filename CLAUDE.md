...
- **Inline images:** answers can show Help Center photos — `image` blocks in `INTENTS`, and `[[IMG:<file>.webp|caption]]` markers in AI replies drawn from the curated catalogue in `BASE_FACTS`. The widget validates markers hard (bare `slug-N.webp` under `help-img/` only, max two) and links each figure to its source article. Details: `docs/chat-assistant.md`.
