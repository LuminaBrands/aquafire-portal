...
- **Installer field practice is field practice, not a spec change.** The Enclosure Guide's `#field-notes` section (and the chat's `downdraft` intent) carries what the install crews do on site: cutout **width** at nominal + ⅜″, seal the enclosure against downdrafts, tape as the fallback, seal the DP kit, keep the vents clear. The calculator itself still reports the **published** dimensions from the 2026 install guides (`MODELS` in `app.js`, W = nominal + ¼″) — the field number appears alongside it as `#framing-w`, derived from `dims.w + 0.125`, never in place of it. Source: `docs/source-material/note-installer-field-tips.txt`, which also records the one unresolved ambiguity (⅜″ overall vs per side).
