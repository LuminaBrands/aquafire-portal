...
```
Home → Guides ▾ (Getting Started, Quick Start, Build Yours, Enclosure Guide) → Care ▾ (Water Care, Maintenance, Troubleshoot) → Find a Dealer → Support
```

The nav logo links to the storefront (`https://www.aquafire.com`), not the portal homepage — use the **Home** nav item for that. `maintenance.html` and `rewards.html` also append a "Shop Aquafire" `.nav-cta`. When a page inside a dropdown is active, its link gets `.active` and the parent `.nav-drop-btn` gets `.active` too.
