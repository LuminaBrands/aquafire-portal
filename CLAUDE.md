...
- **Chat follow-up email capture** — after any contact card or dead end, the widget offers (once per conversation, gated by `state.emailAsked`) a one-field form to leave an email for a human follow-up; disable with `AQUAFIRE_ASSISTANT_CONFIG.collectEmail = false`. Submitting fires the `callback` Slack alert (the address rides a dedicated field, unmasked) and logs a `contact_left` telemetry event — **the one consented exception to the "no customer emails in telemetry" rule**; the incidental-email masks on `text`/`comment` and on all other alert kinds stay in force. Chat Insights tags identified conversations with the address. The `email` field must stay in the `chatEvents` allowlist in `docs/firestore-rules.md` (hand-published — republish in the console when it changes).
