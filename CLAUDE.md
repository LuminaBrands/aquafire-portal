...
| `api/notify-slack.js` | **Vercel serverless function** that Slack-alerts Ember's dead ends to `#chat-insights-feeback` — unanswered questions, AI "I don't know" replies, AI outages, and human handoffs; handoffs once per conversation, emails masked; needs `SLACK_WEBHOOK_URL` env var, 503s gracefully until set |
