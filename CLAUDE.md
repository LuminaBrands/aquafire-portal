...
- **troubleshoot.css carries no fallbacks any more.** It used to read `var(--blue, #4da6e8)` everywhere, from when the page had a partial inline `:root`. `troubleshoot.html` loads `redesign.css` and has no inline `:root`, so every fallback was dead — and each was a 2025, dark-only value that would silently reinstate the old palette if a token were renamed. Stripped Aug 2026 along with the remaining literals. Note the neutral fills there mix from `--text` rather than using `--surface-alt`: a neutral chip has to *darken* on a light card, and `--surface-alt` is near-white in that theme.
