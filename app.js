...
(function initTour() {
  const STOPS = [
    {
      title: 'Water Fill Port',
      desc: 'Located on the back panel. Use this port to manually add water to the reservoir. The unit also supports direct plumbing for automatic fill.',
      view: 'back',
      hotspot: 'hotspot-fill',
    },
    {
      title: 'Control Panel',
      desc: 'Three buttons on the back panel: Power (turns unit on/off), B1 (starts the fill cycle), and B2 (starts the drain cycle). The power LED glows red when active.',
      view: 'back',
      hotspot: 'hotspot-ctrl',
    },
    {
      title: 'LED Light Bar',
      desc: 'A full-width LED strip runs along the inside top edge of the front opening. It illuminates the water vapor to create realistic flame effects with adjustable color.',
      view: 'front',
      hotspot: 'hotspot-led',
    },
    {
      title: 'Venting',
      desc: 'Slot vents run along both sides of the unit in upper and lower bands. These allow airflow for the ultrasonic mist system. Keep vents unobstructed in your enclosure.',
      view: 'side',
      hotspot: 'hotspot-vents',
    },
    {
      title: 'Power Port',
      desc: 'The DC power input is on the back panel, lower-left area. Connect the included power adapter here. Route the cable out through your enclosure before finishing.',
      view: 'back',
      hotspot: 'hotspot-power',
    },
    {
      title: 'Drain Port',
      desc: 'Located on the back panel near the bottom. Connect tubing here to drain the reservoir for maintenance or winterization. Can also be plumbed to a floor drain.',
      view: 'back',
      hotspot: 'hotspot-drain',
    },
  ];

  let current = 0;
  const titleEl = document.getElementById('tour-stop-title');
  const descEl = document.getElementById('tour-stop-desc');
  const countEl = document.getElementById('tour-stop-count');
  const labelEl = document.getElementById('tour-view-label');
  ...
  STOPS.forEach(function(_, i) {
    ...
    dot.className = 'tour-dot' + (i === 0 ? ' tour-dot--active' : '');
    dot.setAttribute('aria-label', 'Go to stop ' + (i + 1));
    dot.addEventListener('click', function() { goTo(i); });
    dotsEl.appendChild(dot);
  });

  var VIEW_LABELS = { front: 'Front View', back: 'Back View', side: 'Side View' };
  ...
  function goTo(idx) {
    ...
    countEl.textContent = (current + 1) + ' / ' + STOPS.length;

    document.querySelectorAll('.tour-svg').forEach(function(svg) {
      svg.classList.remove('tour-svg--active');
    });
    var activeView = document.getElementById('tour-view-' + stop.view);
    if (activeView) activeView.classList.add('tour-svg--active');
    labelEl.textContent = VIEW_LABELS[stop.view] || '';

    document.querySelectorAll('.tour-hotspot').forEach(function(h) {
      h.style.display = 'none';
    });
    var hotspot = document.getElementById(stop.hotspot);
    if (hotspot) hotspot.style.display = '';

    dotsEl.querySelectorAll('.tour-dot').forEach(function(d, i) {
      d.classList.toggle('tour-dot--active', i === current);
    });

    prevBtn.disabled = current === 0;
    nextBtn.disabled = current === STOPS.length - 1;
  }

  prevBtn.addEventListener('click', function() { if (current > 0) goTo(current - 1); });
  nextBtn.addEventListener('click', function() { if (current < STOPS.length - 1) goTo(current + 1); });

  document.getElementById('tour-section').addEventListener('keydown', function(e) {
    if (e.key === 'ArrowLeft' && current > 0) goTo(current - 1);
    if (e.key === 'ArrowRight' && current < STOPS.length - 1) goTo(current + 1);
  });
